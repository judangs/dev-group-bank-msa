...
import org.bank.pay.global.swagger.spec.PaymentCallbackSwaggerSpec;
...
public class PaymentController implements PaymentCallbackSwaggerSpec {
    ...
    @GetMapping("/{user}/{cardId}/{reserveId}")
    public ResponseEntity<? extends ResponseDtoV2> callback(@PathVariable AuthClaims user,
        ...
        if(resultCode.equals("Success")) {
            ResponseDtoV2 response = paymentService.process(user, cardId, paymentId);
            return httpResponseEntityStatusConverter.convert(response);
        }
