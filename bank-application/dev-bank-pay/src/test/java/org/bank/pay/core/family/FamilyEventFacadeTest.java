...
class FamilyEventFacadeTest {
    ...
    public void setup() {
        ...
        payOwnerStore.save(new PayOwner(leader));
    ...
    void 새로운_멤버를_초대합니다() {
        ...
        Assertions.assertThatCode(() -> familyEventFacade.invite(family.getFamilyId(), leader, follower.getUserid()))
    ...
    void 가족_구성원이_결제를_요청합니다() {
        ...
        Assertions.assertThat(family).isNotNull();
        Assertions.assertThatCode(() -> familyEventFacade.purchaseProductToFamilyLeader(family.getFamilyId(), participant.getUserid(), new Product("예제 상품", 1000, 1)))
                .doesNotThrowAnyException();
    }
